...
return (
    ...
    className='relative flex w-full focus:outline-none h-[14vh] ' onClick={()=>setClick(oldState=>!oldState)} onKeyDown={(e)=>{if (e.code==='Enter'){setClick(oldState=>!oldState)}}}>
      <Link title='Go to the brand website' onFocus={()=>{setFocusing(true)}} onBlur={()=>{setFocusing(false)}} className='cursor-alias w-full focus:outline-none' target='_blank' rel='noopener noreferrer' href={link}>

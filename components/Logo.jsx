...
return (
    ...
    className='relative flex w-full h-[14vh] ' onClick={()=>setClick(oldState=>!oldState)}>
      <Link className=' cursor-alias w-full ' target='_blank' rel='noopener noreferrer' href={link}>
        <div className={`flex absolute w-full p-6 sm:p-4 top-1/2 left-1/2 -translate-x-1/2  -translate-y-1/2 duration-700 ${hovering?'opacity-100':'opacity-0'}`}>{color}</div>
        <div className={`flex absolute w-full p-6 sm:p-4 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 duration-700 ${hovering?'opacity-0':'opacity-100'}`}>{white}</div>
      </Link>
        {/* {click?color:''} */}
